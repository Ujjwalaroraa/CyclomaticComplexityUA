...
class Slave:
    '''
    A Slave calculates the average CycComp for a given commit in a Github repo
    The Master issues each Slave with a repo URL upon instantiation, and a
    commit SHA each time it asks for work. The Slave performs CC analysis on
    ...
    def do_work(self):
        '''
        Slave continuously asks Master for work until all work has been exhausted
        Slave is told to finish either when the Job Queue on the Masters end is empty
        or this Slave was the last to submit it's work.
        '''
    ...
    def get_work(self):
        ...
        Slave asks Master for a commit SHA. Any response code
